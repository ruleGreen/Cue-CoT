...
function updateQuestionSelect(question_id) {
    ...
    category = questionMapping[question_id].dataset;
...
function populateQuestions(questions) {
    ...
    questions.forEach(question => {
        ...
        if (question.dataset in categoryMapping) {
            // console.log(categoryMapping)
            categoryMapping[question.dataset].push(question.id);
        } else {
            categoryMapping[question.dataset] = [question.id];
            const category_option = document.createElement('option');
            category_option.value = question.dataset;
            category_option.textContent = capitalizeFirstChar(question.dataset);
...
function displayAnswers(index) {
    ...
    document.getElementById('our-model-answer').innerHTML = text2Markdown(question.answers.ours);

    // Display evaluation
    score = question.scores[otherModel];
    // score_text = modelNameMapping[otherModel] + " " + score[0] + "/10, Ours " + score[1] + "/10";
    score_text = score[0] + "/10, Ours " + score[1] + "/10";
    ...
    let assistant2_title = "Assistant #2 (our method)";
...
function switchQuestionAndCategory() {
    ...
    new_category = questionMapping[currentQuestionIndex].dataset;
