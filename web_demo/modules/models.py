...
class OpenAIClient(BaseLLMModel):
    ...
    def _get_response(self, stream=False):
        ...
        # build system prompt according to user status
        if self.user_status == "infer":
            system_prompt = "Based on the conversation between the user and the assistant, try to infer the user's personality traits, psychological and emotional states exhibited in the conversation as much as possisble using a third-person wording briefly. "
        elif system_prompt != "" and system_prompt != None and "user_status" in system_prompt:
            system_prompt = system_prompt.format(user_status=self.user_status)
        ...
        if system_prompt is not None:
            ...
            history = [*history, construct_system(system_prompt)]
...
class ModelManager:
    ...
    def set_user_status(self, *args):
        return self.model.set_user_status(*args)
